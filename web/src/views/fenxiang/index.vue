...
export default {
  ...
  methods: {
      ...
      console.log("这里是个方法码",row.tree,that.dialogVisible)
        //TODO:根据row显示不同表单  123单株   4 5 古树群
         ...
         that.tree_type = row.tree.tree_type
          ...
          for(let i in that.tree_details.images){
          console.log("是这里吗",that.tree_details.images[i])
          // for(let mm in this.tree_details[i].images){
            this.tree_details.images[i] = 'http://39.105.11.4:81/backend/' + this.tree_details.images[i]
    ...
    pass({ index, row }) {
      ...
      console.log("pass");
      return request({
        url: "/api/system/audit_province/" + id + "/pass/",
        method: "post",
        data: row,
      }).then((res) => {
        console.log(res);
    ...
    notpass({ index, row }) {
      const id = row.id;
      if(row.reject_opinion){
        console.log('notpass',row);
        return request({
          url: "/api/system/audit_province/" + id + "/unpass/",
          method: "post",
          data: row,
        }).then((res) => {
          console.log(res);
